# BackEnd/app/domain/profiling/interaction_style_engine.py
# -*- coding: utf-8 -*-
...
from app.data_access.profiling.interaction_style_repository import (
...
class InteractionStyleEngine(BaseEngine):
    """
    ...
    def analyze(self, learner_uids: List[str]) -> Dict[str, Any]:
        ...
        if not self.ensure_initialized():
            logger.error("InteractionStyleEngine 初始化失败。")
            return {}
    ...
    def _build_dimension_result_for_learner(
        ...
        # 最终 style：出现次数最多；并列时按优先级（2>1>0），同优先级时 code 越大越好
        final_style_code = choose_final_code(
            list(style_codes.values()),
            code_priority={0: 0, 1: 2, 2: 3},
        )
