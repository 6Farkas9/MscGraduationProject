# BackEnd/app/domain/profiling/attention_allocation_engine.py
# -*- coding: utf-8 -*-
...
from app.data_access.profiling.attention_allocation_repository import (
    AttentionAllocationRepository,
)
from app.domain.common.base_engine import BaseEngine
from app.shared.utils.stats_utils import (
    compute_mean_std,
    kmeans_1d,
    choose_final_code,
)
...
def classify_style_code(metrics: Dict[str, float]) -> int:
    ...
    与 profiles_labels.attention_allocation.style 定义保持一致。
    """
...
# Engine 主体
# ----------------------------------------------------------------------
class AttentionAllocationEngine(BaseEngine):
    """
    注意力分配画像分析引擎。
    ...
    def __init__(
        ...
        """
        Args:
            device: 运行设备标记（保留与 BaseEngine 统一接口，尽管当前分析主要在 CPU 上完成）。
    ...
    def initialize(self) -> bool:
        """
        当前引擎没有额外的模型权重或大型结构需要加载，
        但仍保留初始化流程以便未来扩展（例如加载配置/外部模型等）。
        """
        if self.is_initialized:
            return True

        # 若未来需要进行仓库连通性自检，可在此处添加简单查询 / ping。
    ...
    def analyze(self, learner_uids: List[str]) -> Dict[str, Any]:
        ...
        if not self.ensure_initialized():
            logger.error("AttentionAllocationEngine 未能完成初始化，分析终止。")
            return {}
...
def _print_with_text_labels(engine_result: Dict[str, Any]) -> None:
    ...
    仅用于简单手工测试 / 调试，不建议在生产路径中直接调用。
    """
    # 在测试场景下导入标签配置
    from app.shared.models.profiles_labels import get_label  # type: ignore[import]  # 依赖外部模块路径
