# BackEnd/app/domain/profiling/feedback_orientation_engine.py
# -*- coding: utf-8 -*-
...
from app.data_access.profiling.feedback_orientation_repository import (
...
class FeedbackOrientationEngine(BaseEngine):
    """
    ...
    def analyze(self, learner_uids: List[str]) -> Dict[str, Any]:
        ...
        if not self.ensure_initialized():
            logger.error("FeedbackOrientationEngine 初始化失败。")
            return {}
    ...
    def _analyze_per_course(
        ...
        for crs_uid, entries in course_entries.items():
            ...
            cluster_ids = kmeans_1d(FO_norm, k=k, max_iter=50)
            ...
            for idx, (lrn_uid, metrics) in enumerate(entries):
                ...
                per_lc_result[key] = {
                    ...
                    "feedback_view_rate": float(
                        metrics.get("feedback_view_rate", 0.0)
                    ),
                    "support_view_rate": float(
                        metrics.get("support_view_rate", 0.0)
                    ),
