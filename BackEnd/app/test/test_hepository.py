...
def test_hgc_repository():
    ...
    try:
        ...
        if hgc_data:
            ...
            logger.info(f"  - 单元映射大小: {len(hgc_data['unit_uid_mapping'])}")
            logger.info(f"  - 主题映射大小: {len(hgc_data['topic_uid_mapping'])}")
            logger.info(f"  - 课程映射大小: {len(hgc_data['course_uid_mapping'])}")
            logger.info(f"  - 使用策略: {hgc_data['strategy_used']}")
            
            # 验证目标学习者是否在索引0
            if test_learner_uid in hgc_data['learner_uid_mapping']:
                target_idx = hgc_data['learner_uid_mapping'][test_learner_uid]
                logger.info(f"  - 目标学习者索引: {target_idx} {'✓' if target_idx == 0 else '✗'}")
            else:
                logger.warning("  - 目标学习者不在映射中!")
...
def test_cd_repository():
    ...
    try:
        ...
        if cd_data:
            ...
            logger.info(f"  - 涉及题目数: {len(cd_data['involved_questions'])}")
            logger.info(f"  - 题目映射大小: {len(cd_data['question_uid_mapping'])}")
            logger.info(f"  - 学习者映射大小: {len(cd_data['learner_uid_mapping'])}")
            
            # 验证目标学习者是否在索引0
            if test_learner_uid in cd_data['learner_uid_mapping']:
                target_idx = cd_data['learner_uid_mapping'][test_learner_uid]
                logger.info(f"  - 目标学习者索引: {target_idx} {'✓' if target_idx == 0 else '✗'}")
            else:
                logger.warning("  - 目标学习者不在映射中!")
...
def test_kt_repository():
    ...
    try:
        ...
        if kt_data:
            ...
            logger.info(f"  - 涉及学习单元数: {len(kt_data['involved_units'])}")
            logger.info(f"  - 单元映射大小: {len(kt_data['unit_uid_mapping'])}")
            logger.info(f"  - 单元类型映射大小: {len(kt_data['unit_types_mapping'])}")
            logger.info(f"  - 学习者映射大小: {len(kt_data['learner_uid_mapping'])}")
            
            # 验证目标学习者是否在索引0
            if test_learner_uid in kt_data['learner_uid_mapping']:
                target_idx = kt_data['learner_uid_mapping'][test_learner_uid]
                logger.info(f"  - 目标学习者索引: {target_idx} {'✓' if target_idx == 0 else '✗'}")
            else:
                logger.warning("  - 目标学习者不在映射中!")
...
def test_multiple_learners():
    ...
    try:
        ...
        test_learner_uids = ["lrn_004a9c3f5bf246faab3d390ce716e658", "lrn_efc1639eb8ec4421a7559ad4d2f9858c"]
        ...
        # 验证每个学习者的索引位置
        for learner_uid in test_learner_uids:
            if learner_uid in cd_batch_data and cd_batch_data[learner_uid]:
                cd_data = cd_batch_data[learner_uid]
                target_idx = cd_data['learner_uid_mapping'].get(learner_uid, -1)
                logger.info(f"  - {learner_uid} 在CD中的索引: {target_idx} {'✓' if target_idx == 0 else '✗'}")
            
            if learner_uid in kt_batch_data and kt_batch_data[learner_uid]:
                kt_data = kt_batch_data[learner_uid]
                target_idx = kt_data['learner_uid_mapping'].get(learner_uid, -1)
                logger.info(f"  - {learner_uid} 在KT中的索引: {target_idx} {'✓' if target_idx == 0 else '✗'}")
