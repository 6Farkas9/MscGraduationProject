...
from app.repositories.base_repository import BaseRepository  # 基类
...
class AttentionAllocationRepository(BaseRepository):
    ...
    只负责：
    - 利用 Interaction 集合中现有索引，筛选指定学习者相关的课程；
    - 在 MongoDB 层按 (学习者, 课程) 聚合出分析所需的基础统计量；
    - 不做任何标签、聚类和高层分析。

    返回的数据由 engine 进一步计算比例、E_att、聚类结果等。
    """
    ...
    def __init__(
        ...
        """
        Args:
            batch_size: Mongo 游标的 batch_size，控制单次从服务器取多少文档。
            course_chunk_size: 每次查询时 _course_uid 的分片大小，避免一次 $in 太多课程。
        """
        super().__init__()
    ...
    def load_metrics_for_learners(
        ...
        if not learner_uids:
            logger.info("AttentionAllocationRepository.load_metrics_for_learners: 空的学习者列表，直接返回。")
            return {}, {}, {}
    ...
    def _iterate_events(
        ...
        为了利用已有索引（idx_course_verb_lrn），match_query 中必须至少包含:
            - "_course_uid": 某值或 {"$in": [...]}，且
            - "verb.id": 某值或 {"$in": [...]}
        """
        col = self._get_interaction_collection()
        cursor = (
    ...
    def _aggregate_raw_data_for_courses(
        ...
        # ---------- 6.1 按课程分片处理 focused-on-resource ----------
        ...
        # ---------- 6.4 汇总为 (lrn, crs) 的基础统计 ----------
        # 首注视计数：从 first_aoi 抽取
        first_counts: Dict[
            Tuple[str, str],
            Dict[str, int],
