...
class FeedbackOrientationRepository(BaseRepository):
        ...
        for chunk_idx in range(total_chunks):
            ...
            for doc in self._iterate_events(match_query, projection):
                ...
                # 即时支持使用（requested-support）
                    ...
                    # 原脚本中 support 不计入“反馈后正确率提升”的时间线，所以这里不加入 timeline
    ...
    def _compute_metrics_from_raw(
        ...
        for key, stat in raw_stats_by_lc.items():
            ...
            if opportunity_count <= 0 and feedback_view_count <= 0 and support_view_count <= 0:
                continue
            ...
            # 3) 反馈后正确率提升（与原脚本相同逻辑，只在 Repository 里计算数值）
