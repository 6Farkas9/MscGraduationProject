...
class TaskEfficiencyRepository(BaseRepository):
    ...
    # ------------------------------------------------------------------
    # 工具：duration 解析 & 均值/标准差
    # ------------------------------------------------------------------
    @classmethod
    def _parse_iso8601_duration(cls, duration_str: Any) -> Optional[float]:
        """
        解析 PT{n}S 形式的 duration，返回秒数（float）。
        格式不符或空则返回 None。
        """
    ...
    def _compute_efficiency_for_courses(
        ...
        逻辑与原 analyze_task_efficiency.py 一致：:contentReference[oaicite:1]{index=1}
