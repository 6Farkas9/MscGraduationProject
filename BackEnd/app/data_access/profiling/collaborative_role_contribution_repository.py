# BackEnd/app/data_access/profiling/collaborative_role_contribution_repository.py
# -*- coding: utf-8 -*-
...
from app.shared.utils.repository_mixins import (
...
class CollaborativeRoleContributionRepository(MongoDBBaseRepository):
    """
    协作角色与贡献类型（Collaborative Role & Contribution Type）维度的数据仓库。

    职责：
    - 仅负责从 MongoDB.MLS.Interaction 中读取“协作相关”事件；
    - 利用既有复合索引（_lrn_uid + verb.id + _course_uid、
    ...
    def __init__(
        ...
        Args:
            ...
            mongo_operator: 可注入的 MongoDBOperator 实例，便于测试或自定义连接。
        """
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _get_session_id(doc: Dict) -> str:
        """
        取得协作会话标识：
        ...
        sid = (
            exts.get(CollaborativeRoleContributionRepository.EXT_SESSION_ID_1)
            or exts.get(CollaborativeRoleContributionRepository.EXT_SESSION_ID_2)
    ...
    # ------------------------------------------------------------------
    # 内部工具：基于 MongoDBBaseRepository 的 Mongo 访问
    # ------------------------------------------------------------------
    def _iterate_events(
        self,
    ...
    def _aggregate_session_and_course_metrics_for_courses(
        ...
        for chunk_idx in range(total_chunks):
            ...
            for doc in self._iterate_events(match_query, projection):
                ...
                if verb_id == self.VERBS["co_edited_artifact"]:
                    ...
                    dur = parse_iso8601_duration_seconds(result.get("duration"))
                    if dur:
        ...
        for (sid, crs_uid, _lrn_uid), m in per_session_metrics.items():
            ...
            partic = m["collaborated_count"] + safe_divide(
        ...
        for (sid, crs_uid), totals in session_totals.items():
            ...
            for lrn_uid in learners_in_session:
                ...
                share_contrib = safe_divide(contrib, contrib_total)
                share_partic = safe_divide(partic, partic_total)
                share_trans = safe_divide(trans, trans_total)
