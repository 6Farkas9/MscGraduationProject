# reflection_depth_value_evolution_engine.py
# -*- coding: utf-8 -*-
...
from app.repositories.reflection_depth_value_evolution_repository import (
...
class ReflectionDepthValueEvolutionEngine:
    """
            ...
            for (lrn_uid, metrics), cluster_idx in zip(entries, assignments):
                ...
                per_lc_result[key] = {
                    ...
                    "reflection_index": float(
                        metrics.get("reflection_index", 0.0)
                    ),
                    "reflection_count": int(
                        metrics.get("reflection_count", 0)
                    ),
                    "depth_score_avg": float(
                        metrics.get("depth_score_avg", 0.0)
                    ),
    ...
    def _aggregate_overall_level_metrics(
        ...
        raw_idx_vals = [
            float(m.get("reflection_index", 0.0))
            for m in course_metrics.values()
        ]
        depth_vals = [
            float(m.get("depth_score_avg", 0.0))
            for m in course_metrics.values()
        ]
        ve_vals = [
