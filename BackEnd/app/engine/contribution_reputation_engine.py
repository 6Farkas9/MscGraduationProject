# contribution_reputation_engine.py
# -*- coding: utf-8 -*-
...
class ContributionReputationEngine:
    ...
    - Engine 只返回数值 code，文案由 app.models.profiles_labels 负责映射。
    """
    ...
    def _analyze_per_course(
        ...
        for crs_uid, entries in course_entries.items():
            ...
            cluster_ids = self._kmeans_1d(C_norm, k=k, max_iter=50)
    ...
    def _kmeans_1d(
        ...
        # 不需要重排 cluster id 到大小顺序，因为后续会基于中心重排
        # 这里只返回 assignments
        return assignments
