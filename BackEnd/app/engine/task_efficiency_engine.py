...
from app.repositories.task_efficiency_repository import TaskEfficiencyRepository
...
class TaskEfficiencyEngine:
    ...
    - Engine 只返回数值 code，文本标签通过 app.models.profiles_labels 映射。
    """
    ...
    def _analyze_per_course(
        ...
        for crs_uid, entries in course_entries.items():
            ...
            values: List[float] = []
            for _, m in entries:
                v = float(m.get("efficiency_index_norm", 0.0))
                values.append(v)
    ...
    def _kmeans_1d(
        ...
        """
        一维 k-means 聚类，用于课程内 efficiency_index_norm 聚类。
        """
        import random
