...
from app.repositories.slr_helpseeking_repository import SRLHelpseekingRepository
...
class SRLHelpseekingEngine:
    ...
    def _kmeans_1d(
        ...
        """
        一维 k-means 聚类（Lloyd 算法），用于课程内 srl_help_index 聚类。
        """
        import random
        ...
        for _ in range(max_iter):
            ...
            for i in range(k):
                if clusters[i]:
                    new_centers.append(
                        sum(clusters[i]) / float(len(clusters[i]))
                    )
                else:
