...
bool ConceptItemService::recalculate_kcge_cpt_before_delete(std::string &cpt_uid) {
    ...
    // 获取cpt相关的unt_uid
    std::unordered_set<std::string> unt_uids = mysqlop.get_unt_uid_from_graph_involve_by_cpt_uid(cpt_uid);
    // 获取are_uid的emb_uid
    ...
    // 获取指定unt和指定cpt之间的involve关系
    auto unt_cpt = mysqlop.get_unt_cpt_from_graph_involve_by_unts_cpts(
        unt_uids,
        condition_cpt_uids
    );
    // 获取unt的kcge_emb
    auto unt_emb_dict = mongodbop.get_unt_kcge_by_unt_uid(unt_uids);
    for (auto &unt_uid : unt_uids) {
        unt_uid2idx[unt_uid] = idx;
        x.emplace_back(std::move(unt_emb_dict[unt_uid]));
        for (auto &cpt_dif : unt_cpt[unt_uid]) {
            edge_index[0].emplace_back(idx);
    ...
    std::unordered_map<std::string, std::vector<float>> are_emb_write;
    std::unordered_map<std::string, std::vector<float>> cpt_emb_write;
    std::unordered_map<std::string, std::vector<float>> unt_emb_write;
    are_emb_write[are_uid] = std::move(x_out[0]);
    for (auto &cpt_uid_idx : cpt_uid2idx) {
        cpt_emb_write[cpt_uid_idx.first] = std::move(x_out[cpt_uid_idx.second]);
    }
    for (auto &unt_uid_idx : unt_uid2idx) {
        unt_emb_write[unt_uid_idx.first] = std::move(x_out[unt_uid_idx.second]);
    }
    // 保存新的结果
    mongodbop.update_are_kcge_emb(are_emb_write);
    mongodbop.update_cpt_kcge_emb(cpt_emb_write);
    mongodbop.update_unt_kcge_emb(unt_emb_write);
