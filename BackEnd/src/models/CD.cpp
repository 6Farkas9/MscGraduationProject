...
std::vector<float> CD::forward(const std::string are_uid, const std::string lrn_uid){
    ...
    torch::Tensor r_pred = output_data.toTensor().squeeze(0);
    // 输入model获得r_pred
    std::cout << r_pred.sizes() << std::endl;
    // 不清楚输入的tensor和输出的结果的形状，先运行再说
