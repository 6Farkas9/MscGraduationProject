...
if __name__ == '__main__':
    ...
    kcge_pt_temp_path = os.path.join(kcge_pt_path, 'kcge_temp.pt')
    kcge_pt_temp_path = os.path.normpath(kcge_pt_temp_path)
    kcge_pt_use_path = os.path.join(kcge_pt_path, 'kcge_use.pt')
    kcge_pt_use_path = os.path.normpath(kcge_pt_use_path)

    dtr_add_update = True
    mirt_add_update = True
    kcge_add_update = True

    # 有问题：优化器参数的分开组合
    if os.path.exists(cd_pt_train_path):
        print('CD增量训练')
        check_point = torch.load(cd_pt_train_path, map_location=device)
        model_dtr.load_state_dict(check_point['model_state_dict_dtr'])
        model_mirt.load_state_dict(check_point['model_state_dict_mitr'])
        optimizer.load_state_dict(check_point['optimizer_state_dict'])
        lastloss = check_point['loss']
    else:
        print('CD初始训练')
        dtr_add_update = False

    if os.path.exists(kcge_pt_train_path):
        print('KCGE增量训练')
        check_point = torch.load(kcge_pt_train_path, map_location=device)
        model_kcge.load_state_dict(check_point['model_state_dict'])
        optimizer.load_state_dict(check_point['optimizer_state_dict'])
        lastloss = check_point['loss']
    else:
        print('KCGE初始训练')
    ...
    if os.path.exists(cd_pt_temp_path):
        ...
        model_dtr.load_state_dict(check_point['model_state_dict'])
        optimizer.load_state_dict(check_point['optimizer_state_dict'])
