...
class ContrastivePreTraining(nn.Module):
    """对比学习预训练模块 - 我们的特色：基于HGC嵌入"""
    def __init__(self, embedding_dim, hidden_dim=256, temperature=0.07):
        super().__init__()
...
class RelationTemporalEmbedding(nn.Module):
    ...
    def forward(self, h_lrn, h_unt, h_cpt, add1, add2, type_indices, concept_mapping):
        """
        Args:
            h_lrn: [batch_size, seq_len, emb_dim] 学习者嵌入
            h_unt: [batch_size, seq_len, emb_dim] 学习单元嵌入
        ...
        # 关系嵌入 - 练习-概念级
        relational_concept = torch.zeros_like(h_unt)
        for b in range(batch_size):
            for t in range(seq_len):
                unt_idx = type_indices[b, t].item()
                if unt_idx in concept_mapping:
                    # 获取相关知识点嵌入的平均值
                    related_cpts = concept_mapping[unt_idx]
                    if related_cpts:
        ...
        relational_interaction = self.interaction_projection(
            torch.cat([h_lrn, h_unt], dim=-1)
        )
        
        # 时序嵌入
        temporal_features = torch.stack([add1, add2], dim=-1)  # [batch_size, seq_len, 2]
        temporal_input = torch.cat([h_unt, temporal_features], dim=-1)
        temporal_embedding = self.temporal_encoding(temporal_input)
...
class KnowledgeBaseRetrieval(nn.Module):
    """知识点库检索模块 - 我们的特色：近似知识点库检索"""
    def __init__(self, embedding_dim, concept_num, memory_units=20, memory_dim=32):
        super().__init__()
        self.embedding_dim = embedding_dim
...
class KT(nn.Module):
    """改进的知识追踪模型 - 结合论文思路和我们的特色"""
    def __init__(self, embedding_dim, concept_num, h_lrn, h_unt, h_cpt, concept_mapping):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.concept_num = concept_num
        ...
        self.final_prediction = nn.Sequential(
            nn.Linear(embedding_dim * 2 + concept_num, 512),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 256),
            nn.ReLU(),
        ...
        # 类型特定的输入处理 - 我们的特色：处理6种交互类型
        self.type_embedding = nn.Embedding(6, embedding_dim)
        self.type_specific_networks = nn.ModuleList([
            nn.Sequential(
                nn.Linear(2, 32),  # add1, add2
                nn.ReLU(),
                nn.Linear(32, embedding_dim)
            ) for _ in range(6)
        ])
        
    def set_cd_optimized_ability(self, cd_ability, unt_num):
        """
        设置CD优化后的能力矩阵
        Args:
    ...
    def forward(self, lrn_indices, unt_indices, add1, add2, type_indices, 
                seq_mask, next_question_mask, use_cd_optimization=True, use_contrastive=True):
        """
        Args:
        ...
        # 组合基础特征
        base_features = h_unt_batch + type_features
        ...
        relational_temporal_features = self.relation_temporal_embedding(
            h_lrn_batch, sensory_memory, self.h_cpt, add1, add2, unt_indices, self.concept_mapping
        )
    ...
    def get_concept_mastery(self, lrn_indices, unt_seq_indices, add1, add2, type_indices, seq_mask, unt_num):
        """
        专门获取知识点掌握程度供CD使用
        """
        # 可以在这里处理学习单元和题目的区别
        with torch.no_grad():
            _, concept_mastery = self.forward(
                lrn_indices, unt_seq_indices, add1, add2, type_indices, 
...
def test_improved_kt():
    ...
    # 模拟知识点映射
    concept_mapping = {i: [i % concept_num] for i in range(untqus_num)}
    
    # 创建模型
    model = KT(embedding_dim, concept_num, h_lrn, h_unt, h_cpt, concept_mapping)
    ...
    seq_mask = torch.ones(batch_size, seq_len).float()
    ...
    model.train()
    # 只对有效预测计算损失
    valid_predictions = predictions[next_question_mask.bool()]
    if len(valid_predictions) > 0:
        loss = nn.BCELoss()(valid_predictions.mean(dim=-1), torch.randn_like(valid_predictions.mean(dim=-1)).sigmoid())
        loss.backward()
